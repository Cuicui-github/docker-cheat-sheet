...
## Contents combined from the sources below:
...
| docker pull images | `docker pull <image name>` |
| docker run to create a container from an image | `docker run -it -d <image name>` |
| docker exec to access the running container | `docker exec -it <container id> bash` |
| docker login | `docker login` |
| docker push to push an image to the docker hub repository | `docker push <username/image name>` |
...
| build an image from a docker file | `docker build <path to docker file>` |
...
| create a new image of an edited container on the local system | `docker commit <conatainer id> <username/imagename>` |
